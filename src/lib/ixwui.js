...
IXW.Lib.ModalDialog = function(cfg){
	...
	'<form action="{url}" id="{id}_form" name="{id}_form" enctype="multipart/form-data" method="post" target="{id}_frame">',
		...
		'<input type="file" id="{id}_file" name="file" title="{title}" accept=".json"/>',
...
function getDays4Month(year, month, date, trigger){
	...
	var gtDateEnable;//可选当前日期之后，不可选之前。
	if(trigger){
		gtDateEnable = $XD.dataAttr(trigger, "gtDateEnable");
	...
	function _getTpldate4Day(_d){
		...
		if(gtDateEnable){
			if(dInMS < todayInMS)
				clzArr.push("lt-curDate disable");
		}else{
			if(dInMS > todayInMS)
...
var t_triggerTpl = new IX.ITemplate({tpl: [
	'<div class="ixw-dpt {clz}">',
		'<label>{label}</label>',
		'<a data-href="$ixw.dpt.pick" {attrs} data-format="{fmt}" data-gtDateEnable = "{gtDateEnable}">',
...
function DatePickTrigger(cfg){
	...
	var tpldata = {
		...
		gtDateEnable : $XP(cfg, "gtDateEnable", ""),
		fmt : fmt,
...
function getArrowData(arrow, ifDisable){return {
	clz : (ifDisable? "pgArrow disabled" : "pgArrow"),
	extra : "",
	key : arrow,
	text : arrow === "prev" ? "<" : ">"
};}
...
function onPageChanged(pgEl, idx,clickBtnRefreshFn,e){
	var onchange = paginCmp.get((IX.isString(pgEl) || !isNaN(pgEl))?pgEl : pgEl.id);
	IX.isFn(onchange) && onchange(idx -1, clickBtnRefreshFn,e);
}
...
function pgJump(params, aEl){
	...
	onPageChanged(pgEl, idx, "",params.eventN);
}
